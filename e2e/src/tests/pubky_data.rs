...
use pubky_data::{PubkyDataEncryptor, PubkyKeySet, PubkyDataError, ConversationId, TemporaryLinkId, LinkId, PairContextId};
use pubky_data::snow_crypto::{PUBKY_DATA_MSG_LEN, HandshakePattern};
...
fn cipher_check(plaintext: Vec<u8>, ciphertext: &[u8; PUBKY_DATA_MSG_LEN + 2]) {
	...
	if match_check == plaintext_len { panic!() }
}
...
async fn pubky_data_snow_test_initiator_first() {
	...
	let initiator_session = initiator_signer.signup(&server.public_key(), None).await.unwrap();

	let responder_signer = responder_pubky.signer(Keypair::random());
	let responder_session = responder_signer.signup(&server.public_key(), None).await.unwrap();

	let server_path_string = format!("/pub/data");

	let initiator_keypair = Keypair::random();
	let mut initiator_encryptor = PubkyDataEncryptor::init_encryptor_stack(initiator_keypair.secret_key(), 0, "NN".to_string(), initiator_session.clone(), server_path_string.clone(), initiator_pubky, false).unwrap();

	let responder_keypair = Keypair::random();
	let mut responder_encryptor = PubkyDataEncryptor::init_encryptor_stack(responder_keypair.secret_key(), 0, "NN".to_string(), responder_session.clone(), server_path_string.clone(), responder_pubky, false).unwrap();
	...
	let initiator_key_set = PubkyKeySet::new(Some(initiator_ephemeral_keypair.secret_key()), Some(responder_ephemeral_keypair.public_key()));
	let initiator_temporary_link_id = initiator_encryptor.init_context(initiator_key_set, true, responder_public_key.clone()).unwrap();

	let responder_key_set = PubkyKeySet::new(Some(responder_ephemeral_keypair.secret_key()), Some(initiator_ephemeral_keypair.public_key()));
	let responder_temporary_link_id = responder_encryptor.init_context(responder_key_set, false, initiator_public_key.clone()).unwrap();

	// Initiator sends handshake
	// -> e
	// <- e, ee
	initiator_encryptor.handle_handshake(true, initiator_temporary_link_id, responder_public_key.clone()).await;
	responder_encryptor.handle_handshake(false, responder_temporary_link_id, initiator_public_key.clone()).await;
	initiator_encryptor.handle_handshake(true, initiator_temporary_link_id, responder_public_key.clone()).await;

	assert!(!initiator_encryptor.is_handshake(&initiator_temporary_link_id).is_ok());
	assert!(!responder_encryptor.is_handshake(&responder_temporary_link_id).is_ok());
	...
	initiator_encryptor.send_message(raw_bytes, initiator_link_id.clone()).await;

	let responder_link_id = responder_link_id.unwrap();
	let results = responder_encryptor.receive_message(responder_link_id.clone()).await;
	...
	let raw_bytes = data_payload.as_bytes().to_vec();
	responder_encryptor.send_message(raw_bytes, responder_link_id).await;
...
async fn pubky_data_snow_test_responder_first() {
	...
	let responder_link_id = responder_link_id.unwrap();
	responder_encryptor.send_message(raw_bytes, responder_link_id.clone()).await;

	let initiator_link_id = initiator_link_id.unwrap();
	let results = initiator_encryptor.receive_message(initiator_link_id.clone()).await;
	...
	initiator_encryptor.send_message(raw_bytes, initiator_link_id).await;
...
async fn pubky_data_snow_test_responder_tampering() {
	...
	let initiator_keypair = Keypair::random();
	let mut initiator_encryptor = PubkyDataEncryptor::init_encryptor_stack(initiator_keypair.secret_key(), 0, "NN".to_string(), initiator_session.clone(), server_path_string.clone(), initiator_pubky, true).unwrap();

	let responder_keypair = Keypair::random();
	let mut responder_encryptor = PubkyDataEncryptor::init_encryptor_stack(responder_keypair.secret_key(), 0, "NN".to_string(), responder_session.clone(), server_path_string.clone(), responder_pubky, true).unwrap();
	...
	responder_encryptor.send_message(raw_bytes.clone(), responder_link_id.unwrap()).await;

	let results = initiator_encryptor.receive_message(initiator_link_id.unwrap()).await;
...
async fn pubky_data_snow_test_initiator_tampering() {
	...
	let raw_bytes = data_payload.as_bytes().to_vec();
	println!("RAW BYTES {:?}", raw_bytes);
	initiator_encryptor.send_message(raw_bytes.clone(), initiator_link_id.unwrap()).await;

	let results = responder_encryptor.receive_message(responder_link_id.unwrap()).await;
...
async fn pubky_data_snow_null_message() {
	...
	let raw_bytes = data_payload.as_bytes().to_vec();
	responder_encryptor.send_message(raw_bytes, responder_link_id.unwrap()).await;
...
async fn pubky_data_snow_test_min_max_size_message() {
	...
	responder_encryptor.send_message(raw_bytes, initiator_link_id.unwrap()).await;

	let results = initiator_encryptor.receive_message(responder_link_id.unwrap()).await;
...
async fn pubky_data_snow_test_unknown_pattern() {
	...
	let initiator_keypair = Keypair::random();
	let init_encryptor_ret = PubkyDataEncryptor::init_encryptor_stack(initiator_keypair.secret_key(), 0, "BA".to_string(), initiator_session.clone(), server_path_string.clone(), initiator_pubky, false);
...
async fn pubky_data_snow_test_snow_noise_build_error() {
	...
	let initiator_key_set = PubkyKeySet::new(Some(initiator_ephemeral_keypair.secret_key()), Some(responder_ephemeral_keypair.public_key()));
	let init_context_ret = initiator_encryptor.init_context(initiator_key_set, true, responder_public_key.clone());
...
async fn pubky_data_snow_test_already_existent() {
	...
	let initiator_key_set = PubkyKeySet::new(Some(initiator_ephemeral_keypair.secret_key()), Some(responder_ephemeral_keypair.public_key()));
	let first_init_context_ret = initiator_encryptor.init_context(initiator_key_set.clone(), true, responder_public_key.clone());
	assert!(first_init_context_ret.is_ok());
	let second_init_context_ret = initiator_encryptor.init_context(initiator_key_set, true, responder_public_key.clone());
...
async fn pubky_data_snow_test_XX_pattern_simple() {
	...
	let initiator_keypair = Keypair::random();
	let mut initiator_encryptor = PubkyDataEncryptor::init_encryptor_stack(initiator_keypair.secret_key(), 0, "XX".to_string(), initiator_session.clone(), server_path_string.clone(), initiator_pubky, false).unwrap();

	let responder_keypair = Keypair::random();
	let mut responder_encryptor = PubkyDataEncryptor::init_encryptor_stack(responder_keypair.secret_key(), 0, "XX".to_string(), responder_session.clone(), server_path_string.clone(), responder_pubky, false).unwrap();
	...
	let mut data_payload = String::from("Hello_World_Pubky_Data");
	let raw_bytes = data_payload.as_bytes().to_vec();
	initiator_encryptor.send_message(raw_bytes, initiator_link_id.unwrap()).await;
...
async fn pubky_data_snow_test_XX_pattern_tampering() {
	...
	let initiator_keypair = Keypair::random();
	let mut initiator_encryptor = PubkyDataEncryptor::init_encryptor_stack(initiator_keypair.secret_key(), 0, "XX".to_string(), initiator_session.clone(), server_path_string.clone(), initiator_pubky, true).unwrap();

	let responder_keypair = Keypair::random();
	let mut responder_encryptor = PubkyDataEncryptor::init_encryptor_stack(responder_keypair.secret_key(), 0, "XX".to_string(), responder_session.clone(), server_path_string.clone(), responder_pubky, true).unwrap();
...
async fn pubky_data_snow_test_dual_outbox() {
	...
	let initiator_signer = initiator_pubky.signer(Keypair::random());
	let initiator_session = initiator_signer.signup(&first_server.public_key(), None).await.unwrap();

	let responder_signer = responder_pubky.signer(Keypair::random());
	let responder_session = responder_signer.signup(&second_server.public_key(), None).await.unwrap();
